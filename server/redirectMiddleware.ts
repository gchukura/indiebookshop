...
export function createRedirectMiddleware(storageImpl: IStorage) {
  ...
  // This fixes Warning #11: Orphan pages - location variants
  ...
  if (bookshopSlugMatch) {
    ...
    // Skip if it's numeric (handled by Case 9b)
    if (/^\d+$/.test(requestedSlug)) {
      return next();
    }
    
    // Check if this looks like a location variant (has multiple hyphens)
    // Location variants typically have 3+ parts: "name-city" or "name-books-city"
    ...
    if (parts.length >= 2) {
      try {
        ...
        // If lookup fails, continue - let the route handler deal with 404
...
export function redirectMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  // which has access to storage. This legacy function doesn't handle them.
