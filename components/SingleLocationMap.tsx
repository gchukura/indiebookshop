    ...
    const initMap = async () => {
      try {
        ...
        if (!token) {
          ...
          console.error('Mapbox access token is missing in SingleLocationMap');
          return;
        }
        ...
        if (!isMounted || !mapContainerRef.current) {
          console.warn('Component unmounted or map container ref is null, cannot initialize map');
          return;
        }
        
        // Verify the ref points to an actual DOM element
        if (!(mapContainerRef.current instanceof HTMLElement)) {
          console.error('Map container ref is not an HTMLElement');
        ...
        mapInstance.on('error', (e) => {
          ...
          console.error('Mapbox map error in SingleLocationMap', e.error);
        });
