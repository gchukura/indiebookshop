...
const Home = () => {
  ...
  // Function to get quality-filtered bookshops for featuring
  const getFeaturedBookshops = useCallback((): Bookshop[] => {
    if (!bookshops || bookshops.length === 0) return [];
    
    // First, try to get shops with complete profiles (images + descriptions + features)
    const completeProfiles = bookshops.filter(shop => 
      shop.imageUrl && 
      shop.description && 
      shop.description.length > 150 &&
      shop.featureIds && 
      Array.isArray(shop.featureIds) && 
      shop.featureIds.length >= 2
    );
    
    // If we have enough complete profiles, show those
    if (completeProfiles.length >= 6) {
      const shuffled = [...completeProfiles];
    ...
    // Otherwise, get shops with at least images and descriptions
    const goodProfiles = bookshops.filter(shop => 
      shop.imageUrl && shop.description && shop.description.length > 100
    );
    
    if (goodProfiles.length >= 6) {
      const shuffled = [...goodProfiles];
      for (let i = shuffled.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]];
      }
    ...
    // Fallback: any shops with images
    ...
    // Last resort: any random shops (show 3 instead of 6 if quality is low)
        ...
        <div className="container mx-auto">
          <div className="max-w-5xl mx-auto text-center">
            <h1 className="font-serif text-3xl md:text-4xl lg:text-display font-bold text-white mb-4 md:mb-6">
              Find Independent Bookshops Across America
            </h1>
            <p className="font-sans text-base md:text-body-lg text-gray-100 mb-8 md:mb-10 max-w-4xl mx-auto px-2">
              Explore over 2,000 independent bookshops in all 50 states. Search by location, browse by specialty, or discover shops near you on our interactive map.
            </p>
            ...
            <div className="mb-6">
              <Link href="/directory">
                <Button 
                  className="bg-[#E16D3D] hover:bg-[#d06a4f] text-white rounded-full px-8 md:px-10 py-4 md:py-5 text-lg md:text-xl font-semibold min-h-[56px] shadow-lg"
                >
                  Start Exploring →
                </Button>
              </Link>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {/* By Location */}
              <div className="bg-white rounded-lg p-6 shadow-md hover:shadow-lg transition-shadow border-2 border-transparent hover:border-[#2A6B7C]">
                ...
                <Link href="/directory">
                  <Button 
                    className="w-full bg-[#2A6B7C] hover:bg-[#1d5a6a] text-white rounded-full"
                  >
                    Open Map & Filters
                  </Button>
                </Link>
      ...
      <section className="py-8 bg-white border-y border-gray-200">
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center max-w-4xl mx-auto">
            <div>
              <div className="text-4xl md:text-5xl font-serif font-bold text-[#E16D3D] mb-2">2,000+</div>
              <div className="text-gray-700 font-medium">Independent Bookshops</div>
            </div>
            <div>
              <div className="text-4xl md:text-5xl font-serif font-bold text-[#E16D3D] mb-2">50</div>
              <div className="text-gray-700 font-medium">States Covered</div>
            </div>
            <div>
              <div className="text-4xl md:text-5xl font-serif font-bold text-[#E16D3D] mb-2">Growing</div>
      ...
      <section id="featured-bookshops" className="py-8 md:py-12 lg:py-16">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-lg p-6 md:p-8 lg:p-10">
            <div className="relative border-4 border-[#2A6B7C] rounded-lg p-4 md:p-6 lg:p-8 pt-8 md:pt-6 lg:pt-8 shadow-sm">
            ...
            <div className="max-w-4xl mx-auto">
              ...
              <div className="prose prose-lg prose-p:text-gray-700 mx-auto">
                <p>
                  ...
                  directory of indie bookstores available. Our searchable database connects book lovers with local independent 
                ...
                <p>
                  ...
                  or explore featured shops to discover bookstores with unique character and expert curation. We're continuously 
